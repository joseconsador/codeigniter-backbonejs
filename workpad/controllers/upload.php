...
class Upload extends CI_Controller {
  ...
  function __construct() {
        ...
        $this->setPath_img_upload_folder("uploads/media/");
        $this->setPath_img_thumb_upload_folder("uploads/media/thumbnails/");
        ...
        $this->setPath_url_img_upload_folder(base_url() . "uploads/media/");
        $this->setPath_url_img_thumb_upload_folder(base_url() . "uploads/media/thumbnails/");
  }
  ...
  public function upload_img() {
        ...
        $config['file_name'] = md5($name . time());
       ...
       if ($this->do_upload()) {
           ...
           if (IS_AJAX) {   //this is why we put this in the constants to pass only json data
                $this->load->view('ajax', array('json' => array($info)));
           ...
           // default view on the forum so either set them to blank, or decide how you want them to display.  null is passed.
            ...
            $this->load->view('ajax', array('json' => array($error)));
        }
...
//Get info and Scan the directory
    public function get_scan_files() {
        ...
        $this->load->view('ajax', array('json' => $info));
    }
